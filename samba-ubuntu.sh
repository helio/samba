...
import() { local file="$1" name id
    while read name id; do
        grep -q "^$name:" /etc/passwd || adduser --gecos '' --force-badname --disabled-password --no-create-home --uid "$id" "$name"
...
if [[ $# -ge 1 && -x $(which $1 2>&-) ]]; then
    ...
    exec ionice -c 2 -n 2 smbd -F --debug-stdout --no-process-group </dev/null
fi
